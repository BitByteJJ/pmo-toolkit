...
// AI-powered video script generator for PMO cards
// Generates a ~2-minute conversational script structured as:
// Hook → Core Concept → Why It Works → What It Does → When to Use → When NOT to Use → Call to Action

import type { IncomingMessage, ServerResponse } from 'http';

const SYSTEM_PROMPT = `You are a project management educator writing engaging video scripts for a YouTube/LinkedIn audience.
...
export async function handleAiVideoScript(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (req.method !== 'POST') {
