...
const MAP_ITEMS = [
  ...
  { icon: FileText, color: '#10b981', bg: '#ECFDF5',  label: 'Templates',       sub: '198 fillable templates',         path: '/templates'     },
  { icon: Map,      color: '#f59e0b', bg: '#FEF3C7',  label: 'Learning Journey',sub: '35-day PM game',                 path: '/journey'       },
  { icon: Compass,  color: '#0ea5e9', bg: '#F0F9FF',  label: 'Decision Helper', sub: 'Find the right tool fast',       path: '/decision'      },
  { icon: Route,    color: '#8b5cf6', bg: '#F5F3FF',  label: 'Roadmap',         sub: 'Beginner → Advanced paths',      path: '/roadmap'       },
  { icon: BookMarked, color: '#ec4899', bg: '#FDF2F8', label: 'Glossary',       sub: '120+ PM terms defined',          path: '/glossary'      },
  { icon: BookOpen, color: '#ef4444', bg: '#FFF1F2',  label: 'Case Studies',    sub: 'Real-world PM stories',          path: '/case-studies'  },
  { icon: Search,   color: '#64748b', bg: '#F8FAFC',  label: 'Search',          sub: 'Find any card or term',          path: '/search'        },
];
          ...
          {phase === 'map' && (
            <div className="flex flex-col" style={{ maxHeight: '90vh' }}>
              ...
              <div className="flex items-start justify-between px-5 pt-5 pb-3 shrink-0">
                <div>
                  <h2 className="text-[18px] font-black text-stone-900" style={{ fontFamily: 'Sora, sans-serif' }}>
                ...
                <button
                  onClick={dismiss}
                  className="w-7 h-7 rounded-full flex items-center justify-center bg-stone-100 text-stone-500 hover:bg-stone-200 transition-colors shrink-0 mt-0.5"
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {MAP_ITEMS.map((item) => (
                    ...
                    >
                      ...
                      <div>
                        <div className="text-[11px] font-bold text-stone-800 leading-tight">{item.label}</div>
                        <div className="text-[9px] text-stone-500 mt-0.5 leading-tight">{item.sub}</div>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {/* CTA buttons */}
              <div className="px-4 pt-3 pb-5 shrink-0 border-t border-stone-100 flex flex-col gap-2.5">
                <button
                  onClick={startTour}
                ...
                <button
                  onClick={dismiss}
                  className="w-full py-2.5 rounded-2xl text-[13px] font-semibold text-stone-500 hover:text-stone-700 hover:bg-stone-50 transition-colors"
                >
            ...
            <div className="flex flex-col">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {/* Title */}
                  <h3
                    className="text-[20px] font-black text-stone-900 leading-tight mb-1"
              ...
              <div className="px-5 pt-3 pb-6 flex items-center gap-3">
                {stepIndex > 0 && (
                  <button
                    onClick={prevStep}
                    className="flex items-center justify-center w-10 h-10 rounded-xl bg-stone-100 text-stone-600 hover:bg-stone-200 transition-colors shrink-0"
                ...
                {!isLastStep && (
                  <button
                    onClick={dismiss}
                    className="text-[11px] font-semibold text-stone-400 hover:text-stone-600 transition-colors whitespace-nowrap shrink-0"
                  >
                    Skip tour
                  </button>
                )}
